                        ...
                        <div class="d-flex align-items-end flex-wrap">
                            <div class="me-md-3 me-xl-5">
                                <form method="post" action="{{ route('search') }}">
                                    ...
                                    <div class="container">
                                        <div class="dropdown dropdown-select">
                                            <input type="text" id="fromSerial" name="fromserial"
                                                value="<?php if (isset($fromserial)) {
                                                    echo $fromserial;
                                                } ?>" class="form-control"
                                                placeholder="From Serial Number" aria-label="search"
                                                aria-describedby="search">
                                        </div>
                                        <div class="dropdown dropdown-select">
                                        ...
                                        <div class="dropdown dropdown-select">
                                            <select id="agent"name="agent" class="form-control" style="height:39px;">
                                                ...
                                                @foreach ($agents as $key => $agent)
                                                    <option <?php if($selected_agent == $agent->id ){ ?> selected="selected" <?php } ?>
                                                        value="{{ $agent->id }}">{{ $agent->fullname }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="dropdown dropdown-select">
                                            <input type="text" id="code" name="code" value="<?php if (isset($code)) {
                                                echo $code;
                                            } ?>"
                                    ...
                                    <div class="container">
                                        <div class="dropdown-select">
                                            <button type="submit"
                                                class="btn btn-primary mt-2 mt-xl-0 btn-submit">Submit</button>
                                        </div>
                            ...
                            <div id="">
                                <form>
                                    <button type="button" id="printOrder"
                                        class="btn btn-primary mt-2 mt-xl-0">Print</button>
                                </form>
                            </div>
                        </div>
                        <!-- <div style="margin-top:180px">
    ...
    <script>
        $(document).ready(function() {
            $('#printOrder').on('click', function() {
                ...
                var from_date = $('#fromDate').val() || 'null';
                var to_date = $('#toDate').val() || 'null';
                var fromserial = $('#fromSerial').val() || 'null';
                var toserial = $('#toSerial').val() || 'null';
                var agent = $('#agent').val() || 'null';
                var code = $('#code').val() || 'null';

                window.location.href =
                    `print/${from_date}/${to_date}/${fromserial}/${toserial}/${agent}/${code}`;
